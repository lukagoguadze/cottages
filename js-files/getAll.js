...
const fetchUrl = type 
  ? `../php-data/filteredByType.php?type=${encodeURIComponent(type)}`
  : `../php-data/allData.php`;

fetch(fetchUrl)
.then(response=>response.json())
.then(data=>{
           if (type) {
        document.querySelector(".about").innerHTML = `<div class="about-header">
        <h2>${data[0].house_type_desc}</h2>
      </div>`;
    }
    ...
    <a href="/ka/service/" class="service-card">
